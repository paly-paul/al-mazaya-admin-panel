...
interface ApiTicket extends Omit<Ticket, 'id' | 'reference'> {
  id: string | number
  reference?: string
  ref?: string
  tenant_name?: string
...
function normalizeTicket(ticket: ApiTicket): Ticket {
  ...
  return {
    ...ticket,
    id,
    reference: ticket.reference || ticket.ref || id,
    requester: ticket.requester || ticket.tenant_name,
    vendor_name: ticket.vendor_name || ticket.vendor?.company_name,
...
function buildTicketsResponse(res: unknown): TicketsResponse {
  const source = Array.isArray(res)
    ? res
    : res && typeof res === 'object' && 'tickets' in res && Array.isArray((res as { tickets?: unknown }).tickets)
      ? (res as { tickets: ApiTicket[] }).tickets
      : []
  const tickets = source.map((ticket) => normalizeTicket(ticket as ApiTicket))
  ...
  const tats = tickets.filter((t) => t.avg_tat_hrs).map((t) => t.avg_tat_hrs || 0)
  const avgTat = tats.length ? tats.reduce((a, b) => a + b, 0) / tats.length : 0

  return {
    tickets,
    p1_count: res && typeof res === 'object' && 'p1_count' in res && typeof (res as TicketsResponse).p1_count === 'number' ? (res as TicketsResponse).p1_count : p1,
    p2_count: res && typeof res === 'object' && 'p2_count' in res && typeof (res as TicketsResponse).p2_count === 'number' ? (res as TicketsResponse).p2_count : p2,
    p3_count: res && typeof res === 'object' && 'p3_count' in res && typeof (res as TicketsResponse).p3_count === 'number' ? (res as TicketsResponse).p3_count : p3,
    avg_tat_hrs: res && typeof res === 'object' && 'avg_tat_hrs' in res && typeof (res as TicketsResponse).avg_tat_hrs === 'number' ? (res as TicketsResponse).avg_tat_hrs : avgTat,
  }
}
...
export default function TicketsPage() {
  ...
  const fetchTickets = () => {
    ...
    api.get('/api/tickets')
      .then((res) => {
        setData(buildTicketsResponse(res))
      })
  ...
  const handleClose = async (ticketId: string) => {
    ...
    try {
      await api.patch(`/api/tickets/${ticketId}/close`, { resolution_note: 'Closed by admin' })
      setData(prev => prev ? {
        ...prev,
