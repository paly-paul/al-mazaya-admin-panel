...
interface ApiVendor extends Omit<Vendor, 'id' | 'name' | 'category' | 'score' | 'status'> {
  id: string | number
  name?: string
  company_name?: string
  category?: string
...
function normalizeVendor(vendor: ApiVendor): Vendor {
  ...
  return {
    ...
    category: categories.length ? categories.map(formatCategory).join(', ') : 'Other',
    towers: vendor.towers || vendor.towers_covered || [],
    score: vendor.score ?? 0,
    jobs_30d: vendor.jobs_30d ?? vendor.jobs_completed,
    ...
    onboarded_at: vendor.onboarded_at || vendor.created_at || undefined,
  }
}

function buildVendorsResponse(res: unknown): VendorsResponse {
  const source = Array.isArray(res)
    ? res
    : res && typeof res === 'object' && 'vendors' in res && Array.isArray((res as { vendors?: unknown }).vendors)
      ? (res as { vendors: ApiVendor[] }).vendors
      : []
  const vendors = source.map((vendor) => normalizeVendor(vendor as ApiVendor))
  const onboarding = vendors.filter((v) => v.status === 'onboarding').length
  const belowThreshold = vendors.filter((v) => v.score < 50 || v.status === 'below_threshold').length
...
export default function VendorsPage() {
  ...
  const fetchVendors = () => {
    ...
    api.get('/api/vendors')
      .then((res) => {
        setData(buildVendorsResponse(res))
      })
  ...
  const handleOnboard = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/vendors', {
        ...
        categories: [CATEGORY_API_VALUES[form.category] || form.category.toLowerCase()],
        towers_covered: form.towers.split(',').map(t => t.trim()).filter(Boolean),
  ...
  const handleSuspend = async (vendorId: string) => {
    ...
    try {
      await api.patch(`/api/vendors/${vendorId}/status`, { status: 'suspended' })
      setData(prev => prev ? {
        ...prev,
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {loading ? (
          ...
          <>
            ...
            <MetricCard title="Below Threshold" value={data?.below_threshold ?? 0} subtitle="Score < 60" color="red" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">Towers</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">Score</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">Jobs (30d)</th>
            ...
            <tbody>
              {loading ? (
                ...
                vendors.map((vendor, idx) => (
                  ...
                  >
                    <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{vendor.name}</td>
                    <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{vendor.category}</td>
                    ...
                    <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{vendor.jobs_30d ?? '—'}</td>
