...
interface ApiLead extends Omit<Lead, 'id' | 'score' | 'created_at'> {
  id: string | number
  score?: number | null
  tier?: string
...
const STATUS_OPTIONS = ['new', 'follow_up', 'meeting_set', 'proposal_sent', 'nurture', 'closed_won', 'closed_lost']
...
function buildLeadsResponse(res: unknown): LeadsResponse {
  const source = Array.isArray(res)
    ? res
    : res && typeof res === 'object' && 'leads' in res && Array.isArray((res as { leads?: unknown }).leads)
      ? (res as { leads: ApiLead[] }).leads
      : []
  const leads = source.map((lead) => normalizeLead(lead as ApiLead))
  ...
  return {
    leads,
    total: res && typeof res === 'object' && 'total' in res && typeof (res as LeadsResponse).total === 'number' ? (res as LeadsResponse).total : leads.length,
    hot_count: res && typeof res === 'object' && 'hot_count' in res && typeof (res as LeadsResponse).hot_count === 'number' ? (res as LeadsResponse).hot_count : hot,
    warm_count: res && typeof res === 'object' && 'warm_count' in res && typeof (res as LeadsResponse).warm_count === 'number' ? (res as LeadsResponse).warm_count : warm,
    avg_score: res && typeof res === 'object' && 'avg_score' in res && typeof (res as LeadsResponse).avg_score === 'number' ? (res as LeadsResponse).avg_score : avg,
  }
}
...
export default function LeadsPage() {
  ...
  const fetchLeads = () => {
    ...
    api.get('/api/leads')
      .then((res) => {
        setData(buildLeadsResponse(res))
      })
