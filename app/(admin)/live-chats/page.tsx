...
interface ApiChatSession extends Omit<ChatSession, 'id' | 'channel' | 'last_message' | 'started_at'> {
  id?: string | number
...
function normalizeSession(session: ApiChatSession): ChatSession {
  const lastMessage = typeof session.last_message === 'string'
    ? session.last_message
    : session.last_message?.content || undefined

  return {
    ...session,
    id: String(session.id || session.session_id || ''),
    channel: session.channel || 'Web Widget',
    intent: session.intent || session.use_case,
    last_message: lastMessage,
    started_at: session.started_at || session.created_at || session.updated_at || new Date().toISOString(),
    status: session.status || 'active',
  }
}

function buildLiveChatsResponse(res: unknown): LiveChatsResponse {
  const source = Array.isArray(res)
    ? res
    : res && typeof res === 'object' && 'sessions' in res && Array.isArray((res as { sessions?: unknown }).sessions)
      ? (res as { sessions: ApiChatSession[] }).sessions
      : []
  const sessions = source.map((session) => normalizeSession(session as ApiChatSession))
...
export default function LiveChatsPage() {
  ...
  useEffect(() => {
    api.get('/api/dashboard/live-chats')
      .then((res) => {
        setData(buildLiveChatsResponse(res))
      })
