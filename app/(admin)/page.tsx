...
interface ApiDashboardStats {
  ...
  sla_compliance_pct?: number
  tickets_by_tower?: Array<{ tower: string; count: number }>
  leads_by_score_tier?: Array<{ tier: string; count: number }> | Record<string, number>
  tickets_by_priority?: Array<{ priority: string; count: number }> | Record<string, number>
}

function seriesFromRecord<T extends string>(
  value: Array<Record<T, string> & { count: number }> | Record<string, number> | undefined,
  labelKey: T
...
function normalizeDashboardStats(stats: ApiDashboardStats): DashboardStats {
  return {
    open_tickets: stats.open_tickets ?? 0,
    avg_tat_hrs: stats.avg_tat_hrs ?? stats.avg_tat_hours ?? 0,
    lead_pipeline: stats.lead_pipeline ?? stats.lead_pipeline_count ?? 0,
    active_vendors: stats.active_vendors ?? 0,
    chat_sessions: stats.chat_sessions ?? stats.chat_sessions_today ?? 0,
    sla_compliance_pct: stats.sla_compliance_pct ?? 100,
    tickets_by_tower: stats.tickets_by_tower || [],
    leads_by_score_tier: seriesFromRecord(stats.leads_by_score_tier, 'tier'),
    tickets_by_priority: seriesFromRecord(stats.tickets_by_priority, 'priority'),
  }
}
