...
function normalizeBriefing(data: Briefing | null): Briefing | null {
  ...
  return {
    ...
    alerts: (data.alerts || []).map((alert, index) => ({
      ...alert,
      id: String(alert.id || `${alert.severity}-${index}`),
    })),
  }
}

function actionPath(endpoint: string): string {
  return endpoint.replace(/^(GET|POST|PATCH|DELETE)\s+/i, '')
}
...
export default function BriefingPage() {
  ...
  const fetchBriefing = () => {
    ...
    api.get('/api/briefing/latest')
      .then((data) => {
        if (data && !data.detail) setBriefing(normalizeBriefing(data))
  ...
  const handleAction = async (alert: Alert) => {
    ...
    try {
      await api.patch(actionPath(alert.action_endpoint), alert.action_payload || {})
