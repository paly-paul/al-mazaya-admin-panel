...
interface ApiServiceOrder extends Omit<ServiceOrder, 'id' | 'reference' | 'quote_kd' | 'created_at'> {
  id: string | number
  reference?: string
  ref?: string
  tenant_name?: string
  quote_kd?: number | null
...
function normalizeServiceOrder(order: ApiServiceOrder): ServiceOrder {
  ...
  return {
    ...order,
    id,
    reference: order.reference || order.ref || id,
    clinic: order.clinic || order.tenant_name,
    quote_kd: order.quote_kd ?? order.quote_amount ?? undefined,
    vendor_name: order.vendor_name || order.vendor?.company_name,
    status: order.status || 'pending_approval',
    created_at: order.created_at || '',
    completed_at: order.completed_at || order.updated_at || undefined,
  }
}

function buildServicesResponse(res: unknown): ServicesResponse {
  const source = Array.isArray(res)
    ? res
    : res && typeof res === 'object' && 'orders' in res && Array.isArray((res as { orders?: unknown }).orders)
      ? (res as { orders: ApiServiceOrder[] }).orders
      : res && typeof res === 'object' && 'work_orders' in res && Array.isArray((res as { work_orders?: unknown }).work_orders)
        ? (res as { work_orders: ApiServiceOrder[] }).work_orders
        : []
  const orders = source.map((order) => normalizeServiceOrder(order as ApiServiceOrder))
  const now = Date.now()
  const sevenDaysAgo = now - 7 * 24 * 3600000
  const completed7d = orders.filter((o) =>
    o.status === 'completed' && o.completed_at && new Date(o.completed_at).getTime() > sevenDaysAgo
  ).length
  const revenue7d = orders
    .filter((o) => o.status === 'completed' && o.completed_at && new Date(o.completed_at).getTime() > sevenDaysAgo)
    .reduce((s, o) => s + (o.quote_kd || 0), 0)
  const openCount = orders.filter((o) => ['pending_approval', 'approved', 'in_progress'].includes(o.status)).length

  return { orders, open_count: openCount, completed_7d: completed7d, revenue_7d: revenue7d, avg_quote_time_hrs: 4.2 }
}
...
export default function ServicesPage() {
  ...
  const fetchServices = () => {
    ...
    api.get('/api/work-orders')
      .then((res) => {
        setData(buildServicesResponse(res))
      })
